...
async function run() {
    console.log("Testing BRI completely minimal:");
    await testEndpoint({ order: { amount: 10000, invoice_number: 'INV-' + Date.now() }, virtual_account_info: { expired_time: 60, reusable_status: false }, customer: { name: 'test', email: 'test@example.com' } });
    
    console.log("Testing BRI without virtual_account_info:");
    await testEndpoint({ order: { amount: 10000, invoice_number: 'INV-' + Date.now() }, customer: { name: 'test', email: 'test@example.com' } });
}
run();
