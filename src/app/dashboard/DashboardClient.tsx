...
const GlassCard = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
  <div className={`relative overflow-hidden rounded-2xl shadow-2xl bg-white/50 backdrop-blur-xl border border-white/70 transition-all duration-300 hover:bg-white/60 hover:border-white/80 ${className}`}>
...
export default function DashboardClient({ initialPengumuman }: { initialPengumuman: any[] }) {
  ...
  const showPengumumanPopup = (item: any) => {
    Swal.fire({
      title: item.judul,
      html: `
        <div class="text-xs text-emerald-700 mb-4 pb-3 border-b border-white/60 text-left flex items-center gap-2">
        ...
        <div class="text-sm text-gray-800 text-left whitespace-pre-wrap leading-relaxed max-h-64 overflow-y-auto" style="scrollbar-width: thin;">
          ${item.konten}
        </div>
      `,
      ...
      customClass: {
        popup: 'rounded-3xl border border-emerald-200/50 shadow-2xl backdrop-blur-xl',
        title: 'text-left text-xl text-emerald-900 font-bold',
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6 flex flex-col">
            ...
            {/* Header Welcome Card */}
            <GlassCard className="bg-gradient-to-br from-emerald-100/70 to-white/50 border-emerald-500/40">
              <h1 className="text-3xl font-bold text-emerald-950 drop-shadow-lg mb-2">Ahlan wa Sahlan!</h1>
              <p className="text-emerald-900/90 text-sm md:text-base drop-shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
              <GlassCard className="h-full flex flex-col">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-lg font-bold text-emerald-950">Tentang Kami</h2>
                </div>
                ...
                <div className="mt-5 inline-block bg-white/60 border border-white/60 rounded-lg px-4 py-2 w-max shadow-inner">
                  <span className="text-[11px] text-gray-600 block uppercase tracking-wider">Izin PPIU No:</span>
              ...
              <GlassCard className="h-full flex flex-col">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-lg font-bold text-emerald-950">Visi Kami</h2>
                </div>
            ...
            {/* Misi */}
            <GlassCard>
              <h2 className="text-lg font-bold text-emerald-950 mb-4">Misi Kami</h2>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6 text-sm text-gray-800">
          ...
          <div className="lg:col-span-1">
            <GlassCard className="h-full min-h-[450px] flex flex-col">
              <div className="flex items-center gap-3 mb-5">
                ...
                <h2 className="text-lg font-bold text-emerald-950">Informasi & Update</h2>
              </div>
              <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar">
                {pengumumanList.length > 0 ? (
                    ...
                    <button 
                      ...
                      className={`w-full text-left relative p-4 rounded-xl border transition-all hover:shadow-lg cursor-pointer hover:-translate-y-0.5 ${
                        item.is_penting 
                          ...
                          : "bg-white/50 border-white/60 hover:bg-white/40"
                      }`}
                    >
                      <div className="flex flex-col">
                        <div className="flex justify-between items-start gap-2 mb-2">
                          <h3 className={`font-bold text-sm leading-tight ${item.is_penting ? "text-yellow-400 drop-shadow-sm" : "text-emerald-900"}`}>
                            {item.judul}
                          </h3>
                          {item.is_penting && (
                        ...
                        <span className="text-[10px] font-medium text-gray-600 uppercase tracking-wide flex items-center gap-1">
                  ...
                  <div className="p-6 flex flex-col items-center justify-center text-center h-full text-gray-600 bg-white/40 rounded-xl border border-white/60">
                    <Bell className="w-8 h-8 text-emerald-950/20 mb-3" />
          ...
          <div className="lg:col-span-3 grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Accordion */}
            <GlassCard>
              <h2 className="text-lg font-bold text-emerald-950 mb-6">Informasi & Ketentuan Penting</h2>
              
              <div className="space-y-3">
                {/* Accordion 1: Syarat & Ketentuan Pendaftaran */}
                <div className="border border-white/60 rounded-xl overflow-hidden bg-white/50 backdrop-blur-sm shadow-sm transition-all hover:bg-white/60">
                  <button 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-sm text-emerald-950 drop-shadow-sm">Syarat & Ketentuan Pendaftaran</span>
                    </div>
                    {openAccordion === 'syarat_daftar' ? <ChevronUp className="text-gray-600 w-5 h-5" /> : <ChevronDown className="text-gray-600 w-5 h-5" />}
                  </button>
                  {openAccordion === 'syarat_daftar' && (
                    <div className="px-5 pb-4 pt-1 text-xs text-gray-700 border-t border-white/60">
                ...
                <div className="border border-white/60 rounded-xl overflow-hidden bg-white/50 backdrop-blur-sm shadow-sm transition-all hover:bg-white/60">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <FileText className="w-5 h-5 text-emerald-700" />
                      <span className="font-bold text-sm text-emerald-950 drop-shadow-sm">Syarat & Ketentuan Pembatalan</span>
                    </div>
                    {openAccordion === 'syarat_batal' ? <ChevronUp className="text-gray-600 w-5 h-5" /> : <ChevronDown className="text-gray-600 w-5 h-5" />}
                  </button>
                  {openAccordion === 'syarat_batal' && (
                    <div className="px-5 pb-4 pt-1 text-xs text-gray-700 border-t border-white/60 space-y-3 mt-2">
                      <div>
                        <strong className="text-emerald-950 block mb-0.5">A. Uang Muka (DP)</strong>
                        <p>DP yang telah diserahkan oleh calon jamaah umroh tidak bisa dikembalikan.</p>
                      </div>
                      <div>
                        <strong className="text-emerald-950 block mb-0.5">B. Pelunasan</strong>
                        <ul className="list-disc pl-4">
                          <li>Calon jamaah wajib melakukan pelunasan selambat-lambatnya H-30.</li>
                        </ul>
                      </div>
                      <div>
                        <strong className="text-emerald-950 block mb-0.5">C. Pembatalan</strong>
                ...
                <div className="border border-white/60 rounded-xl overflow-hidden bg-white/50 backdrop-blur-sm shadow-sm transition-all hover:bg-white/60">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-sm text-emerald-950 drop-shadow-sm">Ketentuan Khusus</span>
                    </div>
                    {openAccordion === 'ketentuan_khusus' ? <ChevronUp className="text-gray-600 w-5 h-5" /> : <ChevronDown className="text-gray-600 w-5 h-5" />}
                  </button>
                  {openAccordion === 'ketentuan_khusus' && (
                    <div className="px-5 pb-4 pt-1 text-xs text-gray-700 border-t border-white/60 space-y-3 mt-2">
                      <div>
                        <strong className="text-emerald-950 block mb-0.5">1. Klausul Force Majeure</strong>
                      ...
                      <div>
                        <strong className="text-emerald-950 block mb-0.5">2. Klausul Penyesuaian Harga</strong>
                ...
                <div className="border border-white/60 rounded-xl overflow-hidden bg-white/50 backdrop-blur-sm shadow-sm transition-all hover:bg-white/60">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-sm text-emerald-950 drop-shadow-sm">Alur Pembayaran</span>
                    </div>
                    {openAccordion === 'alur_bayar' ? <ChevronUp className="text-gray-600 w-5 h-5" /> : <ChevronDown className="text-gray-600 w-5 h-5" />}
                  </button>
                  {openAccordion === 'alur_bayar' && (
                    <div className="px-5 pb-4 pt-1 text-xs text-gray-700 border-t border-white/60 mt-2">
                ...
                <div className="border border-white/60 rounded-xl overflow-hidden bg-white/50 backdrop-blur-sm shadow-sm transition-all hover:bg-white/60">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-sm text-emerald-950 drop-shadow-sm">Perlengkapan yang Disediakan</span>
                    </div>
                    {openAccordion === 'perlengkapan' ? <ChevronUp className="text-gray-600 w-5 h-5" /> : <ChevronDown className="text-gray-600 w-5 h-5" />}
                  </button>
            ...
            <GlassCard className="flex flex-col">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <h2 className="text-lg font-bold text-emerald-950 drop-shadow-sm">Lokasi Kantor</h2>
                  </div>
                  <p className="font-semibold text-emerald-700 mb-1 text-sm">PT Madinah Salam Wisata</p>
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer"
                  className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/50 hover:bg-white/70 border border-white/70 rounded-lg text-xs font-bold text-emerald-700 transition-all shadow-sm"
                >
              ...
              <div className="flex-1 rounded-2xl overflow-hidden border border-white/70 shadow-inner bg-white/40 min-h-[250px] relative">
                <iframe 
              ...
              <a 
                  ...
                  target="_blank" 
                  rel="noreferrer"
                  className="sm:hidden mt-4 flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 rounded-xl text-sm font-bold text-emerald-950 transition-all shadow-md"
                >
                  Buka di Google Maps
              </a>
            </GlassCard>
