    ...
    <div className={`relative ${isOpen ? 'z-50' : ''}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-50 mt-1 w-full bg-white border border-garis shadow-md max-h-48 rounded-xl py-1 text-xs overflow-auto focus:outline-none">
          <div
            className={`cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-krem text-gray-900 ${value === "" ? "bg-krem font-bold" : ""}`}
          ...
          {options.map((opt) => (
            <div
              key={opt.value}
              className={`cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-krem text-gray-900 ${value === opt.value ? "bg-krem font-bold" : ""}`}
