  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="w-full bg-white/50 border border-white/70 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm h-[38px] text-emerald-950 backdrop-blur-sm transition-colors hover:bg-white/60"
      >
...
export default function TabunganSearchClient({ pakets, activePaketIds }: { pakets: any[], activePaketIds: string[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="relative p-6 rounded-[2rem] shadow-2xl border border-white/70 bg-white/50 backdrop-blur-xl overflow-hidden">
        <div className="relative z-10">
          <h3 className="text-lg font-bold text-emerald-950 mb-4">Cari Estimasi Paket Tabungan</h3>
          <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
              <label className="block text-sm font-medium text-emerald-950 mb-1">Jenis Pesawat</label>
              <CustomSelect 
                value={pesawat} 
                onChange={setPesawat} 
            ...
            <div>
              <label className="block text-sm font-medium text-emerald-950 mb-1">Jenis Hotel</label>
              <CustomSelect 
                value={hotel} 
                onChange={setHotel} 
            ...
            <div>
              <label className="block text-sm font-medium text-emerald-950 mb-1">Bulan Keberangkatan</label>
              <CustomSelect 
                value={bulan} 
                onChange={setBulan} 
            ...
            <div>
              <button type="submit" className="w-full h-[38px] bg-emerald-600 hover:bg-emerald-700 text-emerald-950 font-medium py-2 px-4 rounded-md transition-colors border border-emerald-500/50">
                Cari Paket
              </button>
            </div>
          </form>
      ...
      {hasSearched && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredPakets.filter(p => !activePaketIds.includes(p.id)).length === 0 ? (
            <div className="col-span-full p-8 text-center text-emerald-950 bg-white/40 rounded-lg border border-white/60 backdrop-blur-sm">
            ...
            filteredPakets
              ...
              const renderStars = (hotelString: string) => {
                ...
                if (match) {
                  ...
                  return <span className="inline-flex items-center gap-0.5">{stars} <span className="text-emerald-950/80 text-[10px] ml-1 font-normal">/Setaraf</span></span>;
                }
                return <span className="text-emerald-950/80 text-xs">{hotelString.replace(/\s*\(Atau Setaraf\)/i, "/Setaraf")}</span>;
              };

              return (
                <div 
                  key={paket.id} 
                  className="relative rounded-[2rem] shadow-2xl hover:shadow-emerald-500/10 transition-all duration-300 border border-white/70 bg-white/50 backdrop-blur-xl overflow-hidden group flex flex-col h-full hover:border-white/40"
                >
                  
                  <div className="relative z-10 p-5 flex flex-col h-full">
                    {/* Title Section */}
                    <div className="mb-4">
                      <div className="inline-flex items-center px-2 py-1 rounded-md bg-emerald-500/20 text-emerald-700 text-[10px] uppercase font-bold mb-3 border border-emerald-500/40">
                    ...
                    <div className="flex-1 space-y-3.5 mb-6">
                      ...
                      <div className="flex gap-3 items-center">
                        ...
                        <div>
                          <p className="text-[10px] text-emerald-950/70 uppercase tracking-wider font-bold">Bulan Keberangkatan</p>
                          <p className="text-sm font-semibold text-emerald-950">{new Date(paket.tanggal_keberangkatan).toLocaleDateString('id-ID', { month: 'long' })}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-3 items-start">
                        ...
                        <div>
                          <p className="text-[10px] text-emerald-950/70 uppercase tracking-wider font-bold mb-0.5">Hotel (/Setaraf)</p>
                          <div className="text-xs font-medium text-emerald-950 flex items-center gap-1 mb-1">Makkah: {renderStars(paket.hotel_makkah || "Bintang 4")}</div>
                          <div className="text-xs font-medium text-emerald-950 flex items-center gap-1">Madinah: {renderStars(paket.hotel_madinah || "Bintang 4")}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-3 items-center">
                        ...
                        <div>
                          <p className="text-[10px] text-emerald-950/70 uppercase tracking-wider font-bold">Maskapai</p>
                          <p className="text-sm font-semibold text-emerald-950">{paket.maskapai || "Menyesuaikan"}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto pt-4 border-t border-white/60">
                      <Link href={`/dashboard/tabungan/baru?paketId=${paket.id}`} className="w-full bg-emerald-600 hover:bg-emerald-700 text-emerald-950 font-bold py-2.5 px-4 rounded-xl shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2 text-sm border border-emerald-500/50">
                        + Pilih Paket Ini
                      </Link>
                    </div>
                  </div>
