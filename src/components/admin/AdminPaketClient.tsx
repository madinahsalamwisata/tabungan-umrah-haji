  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {data.map(item => (
          <div key={item.id} className="relative bg-[#0f1712] border border-white/10 rounded-[2rem] overflow-hidden shadow-2xl flex flex-col md:flex-row group">
            <div className="md:w-2/5 h-48 md:h-auto relative bg-black/50">
              ...
              {item.is_estimasi && (
                <span className="absolute top-4 left-4 bg-yellow-500/90 text-yellow-950 text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider backdrop-blur-sm">Estimasi</span>
              )}
            </div>
            
            <div className="p-6 md:w-3/5 flex flex-col justify-between">
              <div>
                <h3 className="text-xl font-bold text-white mb-2">{item.nama_paket}</h3>
        ...
        {data.length === 0 && (
          <div className="col-span-1 lg:col-span-2 text-center py-12 text-gray-500 italic bg-white/5 rounded-2xl border border-white/10">
          ...
          <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-[#0f1712] border border-white/10 rounded-[2rem] shadow-2xl p-6 sm:p-8 animate-in zoom-in-95 duration-200 custom-scrollbar">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-2 mt-4 p-4 bg-yellow-900/10 border border-yellow-500/20 rounded-xl">
                <input type="checkbox" name="is_estimasi" id="is_estimasi" defaultChecked={editingData?.is_estimasi} className="w-5 h-5 rounded bg-black/40 border-white/10 text-yellow-500 focus:ring-yellow-500" />
