...
export default function AdminJamaahClient({ initialData }: { initialData: JamaahData[] }) {
  ...
  const handleSaveEdit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!editingJamaah || isSubmitting) return;
              ...
              <div className="flex justify-end gap-2.5 mt-6 pt-4 border-t border-garis">
                <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 rounded-xl text-xs font-semibold text-teks-500 bg-krem hover:bg-garis/30 transition-colors">
                 ...
                 <button 
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-semibold text-white bg-hijau-900 hover:bg-hijau-800 disabled:bg-hijau-900/60 disabled:cursor-not-allowed transition-colors shadow-md flex items-center justify-center gap-1.5 min-w-[140px]"
                >
                  {isSubmitting ? (
                    <>
            ...
            <form onSubmit={handleSaveNew} className="space-y-4">
              ...
              <div className="flex justify-end gap-2.5 mt-6 pt-4 border-t border-garis">
                ...
                <button 
                  type="submit" 
                  disabled={isSubmitting}
                  className="px-4 py-2 rounded-xl text-xs font-semibold text-white bg-hijau-900 hover:bg-hijau-800 disabled:bg-hijau-900/60 disabled:cursor-not-allowed transition-colors shadow-md flex items-center justify-center gap-1.5 min-w-[130px]"
                >
                  {isSubmitting ? (
                    <>
                      <svg className="animate-spin h-3.5 w-3.5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
