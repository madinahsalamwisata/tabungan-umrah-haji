...
export default async function ProfilPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      ...
      <div className="relative shadow-2xl rounded-[2rem] overflow-hidden mt-8 bg-black/40 backdrop-blur-xl border border-white/20 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150">
        ...
        <div className="relative z-10 px-6 py-8 sm:px-10 flex flex-col sm:flex-row sm:justify-between sm:items-end border-b border-white/10 gap-4">
          <div>
          ...
          {rencanaAktif.length > 0 ? (
            <div className="grid grid-cols-1 gap-4">
              {rencanaAktif.map((rencana: any) => {
                ...
                return (
                  <div key={rencana.id} className="group relative bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-2xl p-5 transition-all shadow-md overflow-hidden">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`px-2.5 py-1 text-[10px] uppercase tracking-wider font-black rounded-lg shadow-inner ${
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-400">
                          <div className="flex items-center gap-1.5">
                            ...
                            Mulai: {new Date(rencana.tanggal_mulai).toLocaleDateString('id-ID', { month: 'short', year: 'numeric' })}
                          </div>
                          <div className="flex items-center gap-1.5">
                      ...
                      {/* Info Dana */}
                      <div className="sm:text-right shrink-0 bg-black/30 sm:bg-transparent p-3 sm:p-0 rounded-xl sm:rounded-none border sm:border-0 border-white/5">
                        <p className="text-xs text-gray-400 uppercase tracking-widest font-semibold mb-1">Total Terkumpul</p>
                        <p className="text-xl sm:text-2xl font-black text-emerald-400 drop-shadow-sm">
                          {new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(totalTerkumpul)}
                        </p>
                        <p className="text-[10px] text-gray-500 mt-1">
                          dari {new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(rencana.total_biaya)}
                        </p>
                      </div>
            ...
            <div className="text-center py-12 px-4 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-sm">
              ...
              <h3 className="text-lg font-medium text-white mb-2">Belum Ada Tabungan</h3>
              <p className="text-sm text-gray-400 mb-6 max-w-sm mx-auto">Anda belum memiliki riwayat tabungan umrah atau haji. Yuk mulai menabung untuk wujudkan mimpimu ke tanah suci!</p>
              <Link 
                href="/dashboard/tabungan/baru" 
                className="inline-flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-lg hover:shadow-emerald-900/50 transition-all hover:scale-105"
              >
                Mulai Menabung
