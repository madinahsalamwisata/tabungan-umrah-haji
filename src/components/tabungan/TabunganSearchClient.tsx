            ...
            filteredPakets
              ...
              const renderStars = (hotelString: string) => {
                ...
                if (match) {
                  ...
                  return <span className="inline-flex items-center gap-0.5">{stars} <span className="text-emerald-100/70 text-[10px] ml-1 font-normal">/Setaraf</span></span>;
                }
                return <span className="text-emerald-100/70 text-xs">{hotelString.replace(/\s*\(Atau Setaraf\)/i, "/Setaraf")}</span>;
              };

              return (
                <div 
                  key={paket.id} 
                  className="relative rounded-[2rem] shadow-xl hover:shadow-emerald-500/20 transition-all duration-300 border border-emerald-800 bg-emerald-950 overflow-hidden group flex flex-col h-full hover:border-emerald-700"
                >
                  
                  <div className="relative z-10 p-5 flex flex-col h-full">
                    {/* Title Section */}
                    <div className="mb-4">
                      <div className="inline-flex items-center px-2 py-1 rounded-md bg-emerald-500/20 text-emerald-300 text-[10px] uppercase font-bold mb-3 border border-emerald-500/30">
                      ...
                      <h3 className="text-base font-bold text-white leading-snug group-hover:text-emerald-100 transition-colors">
                        {paket.nama_paket}
                      </h3>
                    </div>
                    ...
                    <div className="flex-1 space-y-3.5 mb-6">
                      ...
                      <div className="flex gap-3 items-center">
                        ...
                        <div>
                          <p className="text-[10px] text-emerald-300/80 uppercase tracking-wider font-bold">Bulan Keberangkatan</p>
                      ...
                      <div className="flex gap-3 items-start">
                        ...
                        <div>
                          <p className="text-[10px] text-emerald-300/80 uppercase tracking-wider font-bold mb-0.5">Hotel (/Setaraf)</p>
                          <div className="text-xs font-medium text-white flex items-center gap-1 mb-1">Makkah: {renderStars(paket.hotel_makkah || "Bintang 4")}</div>
                          <div className="text-xs font-medium text-white flex items-center gap-1">Madinah: {renderStars(paket.hotel_madinah || "Bintang 4")}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-3 items-center">
                        ...
                        <div>
                          <p className="text-[10px] text-emerald-300/80 uppercase tracking-wider font-bold">Maskapai</p>
                    ...
                    <div className="mt-auto pt-4 border-t border-emerald-800/50">
                      <Link href={`/dashboard/tabungan/baru?paketId=${paket.id}`} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 px-4 rounded-xl shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2 text-sm border border-emerald-500/50">
                        + Pilih Paket Ini
                      </Link>
                    </div>
                  </div>
