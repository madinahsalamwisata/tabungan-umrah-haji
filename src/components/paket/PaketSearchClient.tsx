  ...
  return (
    <div className={`relative ${isOpen ? 'z-50' : ''}`} ref={dropdownRef}>
      <button
        ...
        className="w-full bg-black/60 backdrop-blur-xl border border-white/10 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm h-[38px] text-white hover:bg-white/15 transition-colors"
      >
...
export default function PaketSearchClient({ pakets, activePaketIds }: { pakets: any[], activePaketIds: string[] }) {
  ...
  return (
    <div className="space-y-6">
      {/* Search Engine */}
      <div className="relative z-30 bg-black/60 backdrop-blur-xl p-6 rounded-[2rem] shadow-2xl border border-white/20 mb-6">
      ...
      <div className="space-y-6">
        {filteredPakets.length === 0 ? (
          ...
          filteredPakets.map((paket) => {
            ...
            return (
              <div key={paket.id} className="bg-black/60 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white/20 overflow-hidden flex flex-col md:flex-row hover:shadow-[0_0_25px_rgba(255,255,255,0.15)] transition-all duration-300 relative w-full group hover:bg-black/70">
