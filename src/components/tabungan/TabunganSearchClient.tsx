        ...
        <div className="space-y-4">
          {filteredPakets.length === 0 ? (
            ...
            filteredPakets.map(paket => {
              ...
              return (
                <div key={paket.id} className="bg-white rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-emerald-100 overflow-hidden relative group flex flex-col">
                  {/* Decorative Left/Top Accent */}
                  <div className="h-1.5 md:h-auto md:w-1.5 w-full absolute top-0 left-0 right-0 md:bottom-0 bg-gradient-to-r md:bg-gradient-to-b from-emerald-600 via-emerald-500 to-yellow-400"></div>
                  
                  <div className="p-6 md:pl-8 flex flex-col md:flex-row md:items-center gap-6">
                    {/* Left content (Title & Details) */}
                    <div className="flex-1">
                      {/* Title Section */}
                      <div className="mb-5 mt-1 md:mt-0">
                        <div className="inline-flex items-center px-2.5 py-1 rounded-md bg-emerald-50 text-emerald-700 text-xs font-semibold mb-2 border border-emerald-100">
                    ...
                    <div className="md:w-56 shrink-0 flex items-center justify-center md:border-l md:border-gray-100 md:pl-6 pt-4 md:pt-0 mt-4 md:mt-0 border-t border-gray-100 md:border-t-0">
                      {isAlreadySelected ? (
                        ...
                        <Link href={`/dashboard/tabungan/baru?paketId=${paket.id}`} className="w-full bg-emerald-900 hover:bg-emerald-800 text-white font-bold py-3 px-4 rounded-xl shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2 text-sm">
                          + Pilih Paket Ini
                        </Link>
                      )}
                    </div>
